...
async function copyDependenciesToFunction({
  ...
  for (const error of result.warnings) {
    if (error.message.startsWith("Failed to resolve dependency")) {
      ...
      if (entryPath === file) {
        console.warn(
          `[@zeabur/astro-adapter] The module "${module}" couldn't be resolved. This may not be a problem, but it's worth checking.`
        );
      } else {
        console.warn(
          `[@zeabur/astro-adapter] The module "${module}" inside the file "${file}" couldn't be resolved. This may not be a problem, but it's worth checking.`
        );
      }
